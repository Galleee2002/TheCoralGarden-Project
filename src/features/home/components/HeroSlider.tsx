...
export function HeroSlider({ slides }: { slides: SlideData[] }) {
  ...
  return (
    <section
      className="relative -mt-16 min-h-[85vh] overflow-x-clip overflow-y-hidden md:min-h-screen"
      ...
      {slides.map((slide, i) => (
        ...
        >
          ...
          {slide.image ? (
            ...
            <div className="absolute inset-0 bg-linear-to-br from-[#042F34] via-[#063B41] to-[#111C24]" />
          )}
          ...
          {/* Content */}
          <div className="relative z-10 mx-auto w-full max-w-screen-2xl px-4 py-24 md:py-32">
            <div className="max-w-5xl">
              <h1 className="font-heading mb-6 text-4xl leading-[1.05] font-black text-white uppercase sm:text-5xl md:mb-8 md:text-[96px]">
                {slide.title}
              </h1>
              ...
              {slide.features ? (
                <ul className="mb-10 flex flex-col gap-4">
                  {slide.features.map((text) => (
                    <li key={text} className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-white/90 sm:text-base md:text-2xl">
                        {text}
                      </span>
                    </li>
                  ))}
                ...
                <p className="mb-8 max-w-2xl text-sm font-medium text-white/90 sm:text-base md:mb-10 md:text-2xl">
                  {slide.description}
                </p>
              ) : null}
